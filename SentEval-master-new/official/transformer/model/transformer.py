...
class Transformer(object):
  ...
  def encode(self, inputs, attention_bias):
    ...
    with tf.name_scope("encode"):
      ...
      #embedded_inputs = tf.Print(embedded_inputs, [embedded_inputs.shape], "embedded_inputs : \n")


      pad_var = embedded_inputs[:,:,:1]

      inputs_padding = model_utils.get_padding( pad_var )
      ...
      with tf.name_scope("add_pos_encoding"):
        ...
        #pos_encoding = tf.Print(pos_encoding, [pos_encoding.shape], "pos_encoding : \n")
        encoder_inputs = embedded_inputs + pos_encoding
        
        #encoder_inputs = tf.Print(encoder_inputs, [encoder_inputs.shape], "encoder inputs: \n")
       # inputs_padding = tf.Print(inputs_padding, [inputs_padding.shape], "inputs_padding : \n")
  ...
  #           length, self.params["hidden_size"])
  #       encoder_inputs = embedded_inputs + pos_encoding
  #       encoder_inputs = tf.Print(encoder_inputs, [encoder_inputs], "encoder inputs: ")
    ...
    for n, layer in enumerate(self.layers):
      ...
      with tf.variable_scope("layer_%d" % n):
        with tf.variable_scope("self_attention"):
          ...
          #print("self_attention encoder_inputs shape ",encoder_inputs.shape)
        with tf.variable_scope("ffn"):
          encoder_inputs = feed_forward_network(encoder_inputs, inputs_padding)
          #print(" encoder_inputs shape normalizered ",encoder_inputs.shape)
